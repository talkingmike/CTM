...
# Added by mfoy: used to trim OS-level handle filename from full directory path
# to simple filename
import ntpath
...
def plot(L, scale=4, dot_size = 3, browser=None):
    ...
    if browser is None:
        browser = _browser
    # Edit by mfoy: trim OS-level handle to file down to simple filename
    # webbrowser.get(browser).open('file://%s' % hpath)
    webbrowser.get(browser).open(ntpath.basename(hpath))
...
# Edit by mfoy: change dir parameter from None to '.' (current working directory)
# def create_temp(suffix='', prefix='tmp', dir=None):
def create_temp(suffix='', prefix='tmp', dir='.'):
